...
class RichPath(private val src: Path) : Path(src) {
    ...
    //TODO: Change to transparent later on
     ...
     */
    var pathDataNodes: Array<PathParser.PathDataNode>? = null
        set(value) {
            value ?: return
    ...
    fun setPathData(pathData: String) {
        PathParser.createNodesFromPathData(pathData)?.let {
            pathDataNodes = it
        }
    }
    ...
    fun inflate(context: Context, xpp: XmlResourceParser) {
        ...
        pathDataNodes = PathParser.createNodesFromPathData(pathData)
